...
namespace UnitTests
{
    ...
    public class AdoTests
    {
        ...
        public void InserirPreco_test() 
        {
            ...
            using (TransactionScope tran = new TransactionScope())
            {
                using (AdoCommand cmd = new AdoCommand())
                {
                    ...
                    preco1.CommandText = "SELECT * FROM Preco WHERE tipo = @tipo AND valor = @valor AND duracao = @duracao AND validade = @validade";
        ...
        {
            String nome = "Baldes", preco = "6,00", duracao = "00:45:00", validade = "3000-10-10 10:00:00";
            using (TransactionScope tran = new TransactionScope())
            {
                using (AdoCommand cmd = new AdoCommand())
                {
        ...
        public void RemoverPromocaoDesconto_test()
        {
            using (TransactionScope tran = new TransactionScope())
            {
                ...
                {
                    SqlCommand promocao = cmd.GetConnection().CreateCommand();
                    promocao.CommandText = "SELECT * FROM PromocaoTemporal WHERE pId = 2";
                    SqlCommand promocao2 = promocao.Clone();
